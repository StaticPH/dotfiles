...
$currentLoc/aliasWinExecs.sh "$dest"	# TODO: Add check for OS; Windows Only
$currentLoc/aliasWin32Execs.sh "$dest"	# TODO: Add check for OS; Windows Only		
...
$currentLoc/genCommonMachineSetup.sh # The files generated by this script will show up in your home directory ($HOME), rather than in the default generated file directory. 

## Note: seemsLikeWSL is located in .setup_base_env
[ $(seemsLikeWSL) ] && $currentLoc/establishWSLConf.sh # This will create a bare-bones wsl.conf file at /etc/wsl.conf, executing itself using sudo if necessary
...
unset dest currentLoc creationFailed